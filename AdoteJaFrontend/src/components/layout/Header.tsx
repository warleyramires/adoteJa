...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-creme-100/90 backdrop-blur-md border-b border-pedra-200">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2 group">
          <span className="font-display text-2xl font-medium text-carbon-800 tracking-tight group-hover:text-ambar-500 transition-colors">
            ...
            <Link
              ...
              className={`font-body text-sm font-medium px-4 py-2 rounded-full transition-colors ${
                location.pathname === link.to
                  ? 'text-ambar-500 bg-ambar-50'
                  : 'text-carbon-800 hover:text-ambar-500 hover:bg-pedra-100'
              }`}
            >
              {link.label}
            </Link>
