...
const statusConfig: Record<StatusSolicitacao, { label: string; variant: 'azul' | 'ambar' | 'neutral' }> = {
  PENDENTE:  { label: 'Pendente',  variant: 'neutral' },
  APROVADA:  { label: 'Aprovada',  variant: 'azul' },
  RECUSADA:  { label: 'Recusada',  variant: 'ambar' },
}
  ...
  return (
    <PageLayout>
      <div className="mb-10">
        ...
        <h1 className="font-display text-5xl font-normal text-carbon-800">Minhas solicitações</h1>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-24">
          <p className="font-display text-3xl font-normal text-carbon-800/30 mb-4">
            Você ainda não fez solicitações
          </p>
          <Link
            to="/pets"
            className="font-body text-sm text-ambar-500 hover:underline"
          >
          ...
          {solicitacoes.map((s) => {
            ...
            return (
              <div key={s.id} className="card p-5 flex items-center gap-5">
                {/* Foto */}
                <div className="w-20 h-20 rounded-2xl bg-pedra-100 overflow-hidden flex-shrink-0">
                  {s.imagemUrl ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {s.especie && (
                      <Badge variant="pedra">{especieLabel[s.especie]}</Badge>
                    )}
                  </div>
